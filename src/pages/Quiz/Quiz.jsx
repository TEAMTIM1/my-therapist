...
const Quiz = () => {
  ...
  const formSubmit = async (event) => {
    ...
    const dataToSend = {
      ...
      answer_3:
        quizData.profile === 'professional' || quizData.target === 'child'
          ? 'none'
          : quizData.subject,
    ...
    try {
      ...
      const response = await axios.post(
        'https://my-therapist-api.up.railway.app/patients/quizz',
        dataToSend
      );
    ...
    <div className="items-center bg-white h-screen">
      ...
      <div className=" grid grid-flow-row md:grid-flow-col  ">
        ...
        <div className=" text-base grid item-center">
          <form onSubmit={formSubmit} className=" max-h-full w-full grid justify-center ">
            {quizArray.map((item, index) => {
              ...
              return (
                <div className=" form-control gap-4 p-4" key={`question-wrapper-${index}`}>
                  ...
                  {item.type === 'radio' && (
                    //J'ai rajouter un div avec grid et grid-cols-2 pour mettre les questions sur 2 colonnes
                      ...
                      {item.answers.map((answer, answerIndex) => (
                        <div key={`question-${index}-${answerIndex}`}>
                          <label className="flex items-center">
                            <input
                              onChange={(event) => {
                                handleChange(event, item.onChange);
                              }}
                              name={item.name}
                  ...
                  {item.type === 'textarea' && (
                    <div key={`question-${index}`}>
                      ...
                      <textarea
                        ...
                        value={quizData[item.name]}></textarea>
                    </div>
                  )}
                </div>
              );
            ...
            {quizData.profile === 'professional' || quizData.gender ? (
              <button
                className="bg-gradient-to-r from-[#A16EE7] to-red-700 rounded-lg text-white p-2 btn m-10"
                type="submit">
                envoyer
              </button>
            ) : null}
